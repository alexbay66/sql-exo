...
-- - date de fin : aucune
    INSERT INTO `school_year` (`id`, `name`, `description`, `start_date`, `end_date`) VALUES (NULL, 'Promo 2023', 'La promo de l\année 2023', DATE('2023-03-15'), NULL); 
-- Exo 2.3
...
UPDATE `tag` SET `description` = 'Le HTML est un language de structuration de données' WHERE `tag`.`id` = 1; 
-- Exo 2.5
-- Mettez à jour le tag `Bash` :
-- nom : Terminal Bash
-- description : Le terminal Bash est l'interface en ligne de commande par défaut sous Linux.
UPDATE `tag` SET `name` = 'Terminal Bash', `description` = ' Le terminal Bash est l\interface en ligne de commande par défaut sous Linux.' WHERE `tag`.`id` = 10; 
-- Exo 2.6<
-- Supprimez la relation entre un student et un tag :
-- - student : Zacharie Evrard
-- - tag : Bash
DELETE FROM student_tag WHERE `student_tag`.`student_id` = 100 AND `student_tag`.`tag_id` = 10 
-- Exo 2.7
-- Supprimez la relation entre un student et un projet :
-- - student : Arthur Lacombe
-- - projet : Dicta quia at qui
DELETE FROM student_tag WHERE `student_tag`.`student_id` = 16 AND `project_tag`.`tag_id` = 6

-- Exo 2.8
-- Supprimez toutes les relations entre un student et ses tags :
-- - student : Odette Thomas
DELETE FROM student_tag WHERE `student_tag`.`student_id` = 99 AND `student_tag`.`tag_id` = 9 
